...
*/
    ...
    float update(Chuck chuck) {
      ...
      if (lastChuckButton == CHUCK_C) {
        ...
        downer->zero();
          ...
          if (settings.HELI_MODE) {
            ...
            if (chuck.Y < -THROTTLE_MIN) {
              ...
              #endif
              throttle = 0;//.5;//Yos changed from 0
              upper->rough(throttle);
		...
		//throttle = upper->smoove(throttle - (throttle - cruiser->getAutoCruise())*0.1);//Changed by Omer 18:16
		  //throttle = -braker->update(-throttle, chuck.X, -chuck.Y);
		  //downer->rough(ABS(throttle));
		  //upper->zero();
        //cruiser->coast();
        ...
        // throttle = upper->smoove(0);
        // drop throttle gently
        //throttle = upper->smoove(throttle*0.75);/Changed by Omer 18:43
        // dump brakes immediately
		//downer->smoove(0);//changed by Omer 18:30
		//cruiser->zero();
		//braker->zero();
